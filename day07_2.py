...
hand_score = {'A':14, 'K':13, 'Q':12, 'T':10, '9':9, '8':8, '7':7, '6':6, '5':5, '4':4, '3':3, '2':2, 'J': 1}

def get_hand_power(hand):
    if hand == 'JJJJJ':
        return 6
    hand_ctr = collections.Counter(hand)
    num_jokers = hand_ctr['J'] if 'J' in hand_ctr else 0
    if num_jokers:
        non_joker_max = sorted([(k,v) for k,v in hand_ctr.items() if k != 'J'], key=lambda kv: kv[1],reverse=True)[0][0]
        hand_ctr = collections.Counter(hand.replace('J',non_joker_max))
